function Star({ overall }) {
  /**
   * Loop how many starts to show based on the overall rating
   */
  const renderStarRating = () => {
    const stars = [];
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <span key={i} className="w-[18px] h-[18px] rounded-[1px]">
          <svg
            width="16"
            height="15"
            viewBox="0 0 16 15"
          ...
          >
            <path
              ...
              fill={`${i <= Math.round(overall) ? "#EDAB00" : "#E6E6E8"}`}
            />
          </svg>
        </span>
      );
